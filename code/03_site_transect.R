...
### west coast AIS data
data_dir <- file.path("data/b_intermediate_data", stringr::str_glue("{region}_ais_transect.gpkg"))
study_dir <- file.path("data/b_intermediate_data/", stringr::str_glue("{mariculture_site}_site.gpkg"))

### output geopackage
output_gpkg <- file.path("data/c_analysis_data/", stringr::str_glue("{mariculture_site}_ais_transect.gpkg"))
...
## study region
study_region <- sf::st_read(dsn = study_dir,
                            layer = sf::st_layers(dsn = study_dir)[[1]][grep(pattern = "study_region",
                                                                        x = sf::st_layers(dsn = study_dir)[[1]])])

#####################################
#####################################

# limit AIS data to the study region
study_ais <- ais_data %>%
...
# export data
sf::st_write(obj = study_ais, dsn = output_gpkg, layer = stringr::str_glue("{region}_{year}{month}_transects"), append = F)
